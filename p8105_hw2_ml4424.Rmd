...
pols_month <- read_csv("prob3_data/fivethirtyeight_datasets/pols-month.csv") %>% 
  janitor::clean_names() %>% 
...
snp_dta <- read_csv("prob3_data/fivethirtyeight_datasets/snp.csv") %>% 
  janitor::clean_names() %>% 
...
```{r tidy unemploy dta}
unemp_dta <- read_csv("prob3_data/fivethirtyeight_datasets/unemployment.csv") %>% 
  janitor::clean_names() %>%
  rename_at(vars(tolower(month.abb)), ~month.name) %>% 
  pivot_longer(January:December,
               names_to = "month") %>% 
  mutate(month = as.character(match(month, month.name)), # coerce month name to month number
         year = as.character(year)) # coerce year to character to match previous datasets
unemp_dta
```
